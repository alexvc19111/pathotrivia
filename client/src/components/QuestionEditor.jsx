...
export default function QuestionEditor({ quizId, question = null, authHeaders, onSaved = () => {}, onCancel = () => {} }) {
  ...
  const [sliderData, setSliderData] = useState({ min: question?.slider_min ?? 0, max: question?.slider_max ?? 100, correct: question?.slider_correct ?? '' })
  const [pinData, setPinData] = useState({ x: question?.pin_x ?? '', y: question?.pin_y ?? '' })
  const [optionsCache, setOptionsCache] = useState({})

  // Sincronización robusta con la prop 'question'
  useEffect(() => {
    if (question) {
      setType(question.type || 'multiple_choice')
      setText(question.question_text || '')
      setTimeLimit(question.time_limit_sec || 20)
      setPoints(question.points || 1000)
      setMediaUrl(question.media_url || '')
      setExplanation(question.explanation || '')
      setSliderData({ min: question.slider_min ?? 0, max: question.slider_max ?? 100, correct: question.slider_correct ?? '' })
      setPinData({ x: question.pin_x ?? '', y: question.pin_y ?? '' })
      
      const raw = question.question_options || []
      const hydrated = raw.map((opt, i) => ({
        id: opt.id || genLocalId(),
        text: opt.option_text ?? opt.optionText ?? opt.text ?? '',
        is_correct: question.type === 'type_answer' ? true : !!(opt.is_correct ?? opt.isCorrect),
        position: opt.position ?? i,
        match_group: opt.match_group ?? opt.matchGroup ?? 'A'
      }))
      setOptionsCache({ [question.type]: hydrated.length > 0 ? hydrated : createDefaultOptions(question.type) })
    } else {
      setOptionsCache({})
    }
  }, [question])

  const currentOptions = useMemo(() => optionsCache[type] || createDefaultOptions(type), [optionsCache, type])
  ...
  const handleAddOption = () => {
    setOptionsForCurrentType(prev => {
      if (type === 'matching') {
        ...
        return [...prev, { id: genLocalId(), text: '', is_correct: false, position: pairIndex, match_group: 'A' }, { id: genLocalId(), text: '', is_correct: false, position: pairIndex, match_group: 'B' }]
      }
      return [...prev, { id: genLocalId(), text: '', is_correct: false, position: prev.length, match_group: 'A' }]
    })
  }

  const handleRemoveOption = (id) => setOptionsForCurrentType(prev => prev.filter(o => o.id !== id))
  const handleChangeOption = (id, field, value) => setOptionsForCurrentType(prev => prev.map(o => o.id === id ? { ...o, [field]: value } : o))
  ...
  const handleSave = async () => {
    ...
    if (needsOptions && validOptions.length === 0) return toast.error('Debes añadir una respuesta correcta válida')

    setLoading(true)
    try {
      const payload = {
        type, question_text: text.trim(), time_limit_sec: timeLimit, points, media_url: mediaUrl || null, explanation: explanation.trim() || null,
        ...(type === 'slider' && { slider_min: Number(sliderData.min), slider_max: Number(sliderData.max), slider_correct: sliderData.correct !== '' ? Number(sliderData.correct) : null }),
        ...(type === 'drop_pin' && { pin_x: pinData.x !== '' ? Number(pinData.x) : null, pin_y: pinData.y !== '' ? Number(pinData.y) : null }),
        question_options: type === 'type_answer' ? [{ ...(typeof currentOptions[0]?.id === 'number' && { id: currentOptions[0].id }), option_text: currentOptions[0]?.text.trim() || '', is_correct: true, position: 0, match_group: 'A' }]
          : validOptions.map((o, idx) => ({ ...(typeof o.id === 'number' && { id: o.id }), option_text: o.text.trim(), is_correct: !!o.is_correct, position: type === 'matching' ? (o.position ?? idx) : idx, match_group: type === 'matching' ? (o.match_group || 'A') : null }))
      }
      const res = await fetch(question ? `${API_URL}/api/questions/${question.id}` : `${API_URL}/api/quizzes/${quizId}/questions`, {
    ...
    <div style={styles.overlay}>
      <div className="card animate-popIn" style={styles.modal}>
        <div style={styles.header}>
          <h2 style={{ fontFamily:'Syne, sans-serif', fontWeight:700, fontSize:'1.25rem' }}>{question ? '✏️ Editar Pregunta' : '➕ Nueva Pregunta'}</h2>
          <button onClick={onCancel} style={styles.closeBtn}><X size={20} /></button>
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
          <div>
            <label style={styles.label}>Tipo de pregunta</label>
            <select value={type} onChange={e => setType(e.target.value)} className="input-field">{Object.entries(QUESTION_TYPES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>
          </div>
          <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
            <div><label style={styles.label}>Tiempo (s)</label><input type="number" value={timeLimit} onChange={e => setTimeLimit(Number(e.target.value))} className="input-field" /></div>
            <div><label style={styles.label}>Puntos</label><input type="number" value={points} onChange={e => setPoints(Number(e.target.value))} className="input-field" /></div>
          </div>
          ...
          {['multiple_choice', 'true_false', 'puzzle', 'matching', 'poll'].includes(type) && <OptionsList type={type} options={currentOptions} onAdd={handleAddOption} onRemove={handleRemoveOption} onChange={handleChangeOption} />}
        </div>
        <div style={{ display:'flex', gap:'0.75rem', marginTop:'1.75rem' }}>
          <button onClick={onCancel} className="btn-ghost" style={{ flex:1 }}>Cancelar</button>
          <button onClick={handleSave} disabled={loading} className="btn-primary" style={{ flex:1 }}>{loading ? '⏳ Guardando...' : '✓ Guardar'}</button>
        </div>
      </div>
    </div>
  )
