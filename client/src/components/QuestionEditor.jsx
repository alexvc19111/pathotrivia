...
export default function QuestionEditor({ quizId, question = null, authHeaders, onSaved = () => {}, onCancel = () => {} }) {
  ...
  // Caché local por tipos para evitar pérdida destructiva si el usuario conmuta el Select accidentalmente
  const [optionsCache, setOptionsCache] = useState(() => {
    const raw = question?.question_options ?? question?.options ?? []
    const initialOptions = raw.length 
      ? raw.map((opt, i) => ({
      ...
      : createDefaultOptions(question?.type || 'multiple_choice')

    return { [question?.type || 'multiple_choice']: initialOptions }
  })
  ...
  // Inicializar tipos faltantes en caché de manera segura y controlada al cambiar el Select
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    if (needsOptions && validOptions.length === 0) {
      return toast.error('Debes añadir al menos una opción válida')
    }
      ...
      const payload = {
        ...
        question_options: needsOptions
          ? validOptions.map((o, idx) => ({
              ...(typeof o.id === 'number' && { id: o.id }), // Mantiene ID de DB si existe
              option_text: o.text.trim(),
              is_correct: !!o.is_correct,
              position: type === 'matching' ? (o.position ?? idx) : idx,
              match_group: type === 'matching' ? (o.match_group || 'A') : null
            }))
          : type === 'type_answer' && currentOptions[0]?.text.trim()
          ? [{ option_text: currentOptions[0].text.trim(), is_correct: true, position: 0, match_group: 'A' }]
          : []
      }
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
          ...
          {type === 'type_answer' && (
            <div>
              ...
              <input type="text" value={currentOptions[0]?.text || ''} 
                ...
                className="input-field" placeholder="Escribe la respuesta esperada..." />
            </div>
          )}
...
/* --- SECCIONES COMPONENTIZADAS AISLADAS --- */
