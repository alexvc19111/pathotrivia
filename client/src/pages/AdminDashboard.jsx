...
import {
  HelpCircle, CheckCircle2, Keyboard, Puzzle, BarChart3, Cloud,
  SlidersHorizontal, Lightbulb, MapPin, GitCompare, Pencil, Trash2
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function startSession() {
    ...
    } catch (err) { toast.error(err.message || \'No se pudo iniciar la sesión\') }
    finally { setStartingSession(false) }
  }

  if (view === \'addQuestion\') {
    // key usa editingQ.id para editar (misma pregunta = mismo componente)
    // y editorKey para nueva pregunta (siempre diferente = siempre remonta limpio)
    return <QuestionEditor
      key={editingQ?.id ?? editorKey}
  ...
  if (view === \'importPdf\') {
    return <PDFImporter quizId={activeQuiz.id} authHeaders={authHeaders}
            ...
            <div className="animate-fadeIn">
              <div style={{ display:\'flex\', alignItems:\'flex-start\', justifyContent:\'space-between\', marginBottom:\'2rem\', flexWrap:\'wrap\', gap:\'1rem\' }}>
                ...
                <div style={{ display:\'flex\', gap:\'0.75rem\', flexWrap:\'wrap\' }}>
                  ...
                  <button className="btn-ghost" onClick={() => { setEditingQ(null); setEditorKey(k => k + 1); setView(\'addQuestion\') }} style={{ fontSize:\'0.85rem\' }}>+ Pregunta</button>
                ...
                {questions.map((q, i) => {
                  ...
                  return (
                    <div key={q.id} className="animate-slideIn" style={{ animationDelay:`${i*0.04}s`, background:\'var(--surface)\', border:\'1px solid var(--border)\', borderRadius:\'14px\', padding:\'1rem 1.25rem\', display:\'flex\', alignItems:\'center\', gap:\'1rem\' }}>
                      <div style={{ width:\'36px\', height:\'36px\', borderRadius:\'10px\', background:\'var(--surface2)\', display:\'flex\', alignItems:\'center\', justifyContent:\'center\', flexShrink:0 }}>
                        {Icon ? <Icon size={18} strokeWidth={2} color={typeInfo?.color || \'#888\'} /> : <HelpCircle size={18} strokeWidth={2} color="#888" />}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:\'flex\', gap:\'0.5rem\', flexShrink:0 }}>
                        <button onClick={() => openEditor(q)} style={{ display:\'flex\', alignItems:\'center\', gap:\'0.4rem\', background:\'rgba(59,130,246,0.12)\', border:\'1px solid rgba(59,130,246,0.25)\', borderRadius:\'8px\', padding:\'0.4rem 0.75rem\', cursor:\'pointer\', color:\'#3b82f6\', fontSize:\'0.8rem\', fontWeight:600 }}>
                          <Pencil size={14} /> Editar
                        </button>
                        <button onClick={() => deleteQuestion(q.id)} style={{ display:\'flex\', alignItems:\'center\', gap:\'0.4rem\', background:\'rgba(239,68,68,0.12)\', border:\'1px solid rgba(239,68,68,0.25)\', borderRadius:\'8px\', padding:\'0.4rem 0.75rem\', cursor:\'pointer\', color:\'#ef4444\', fontSize:\'0.8rem\', fontWeight:600 }}>
